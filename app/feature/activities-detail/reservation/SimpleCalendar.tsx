...
export default function SimpleCalendar({ value, onChange, enabledDateSet }: Props) {
  const today = useMemo(() => new Date(), []);
  const [month, setMonth] = useState(() => new Date(today.getFullYear(), today.getMonth(), 1));

  const isEnabled = (d: Date) => enabledDateSet.has(toISODate(d));
  const isToday = (d: Date) => toISODate(d) === toISODate(today);
  const isSelected = (d: Date) => (value ? toISODate(d) === toISODate(value) : false);

  const weekday1 = (d: Date) => ["일", "월", "화", "수", "목", "금", "토"][d.getDay()];
      ...
      <DayPicker
        ...
        components={{
          MonthCaption: (props: MonthCaptionProps) => {
            ...
            const label = date ? date.toLocaleString("en-US", { month: "long", year: "numeric" }) : "";
            return (
            ...
            return (
              <div className="flex justify-end absolute right-0 top-1 z-1">
                <div className="flex gap-3">
                  ...
                  >
                    <Image src="/icons/icon_arrow_left.svg" alt="" width={24} height={24} />
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <Image src="/icons/icon_arrow_right.svg" alt="" width={24} height={24} />
                  </button>
                </div>
              </div>
            );
